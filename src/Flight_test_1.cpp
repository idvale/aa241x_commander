...
void ControlNode::localPosCallback(const geometry_msgs::PoseStamped::ConstPtr& msg) {
    ...
    // First waypoint to mimic a take off while already in the air
		...
		// check condition on being "close enough" to the waypoint
                if (abs(_current_local_pos_U_lake - _target_U_lake) < 0.5) {
			...
			_wp_index++;
                        _target_E_lake=_current_local_pos_E_lake-20;
        ...
        if (_wp_index == 3) {
                ...
                if (abs(_current_local_pos_E_lake - _target_E_lake) < 0.5) {
                        ...
                        _wp_index++;
                        _target_E_lake=_current_local_pos_E_lake+20;
                        _target_N_lake=_current_local_pos_N_lake+20;
                }
        }
        ...
        if (_wp_index == 4) {
                ...
                // check condition on being "close enough" to the waypoint
                if (abs(_current_local_pos_E_lake - _target_E_lake) < 0.5 && abs(_current_local_pos_N_lake - _target_N_lake)<0.5) {
	...
	while (ros::ok()) {
		...
		if (_wp_index < 0) {
			...
			_wp_index = 0;
                        ...
                        _target_U_lake=_current_local_pos.pose.position.z+_u_offset+5;
