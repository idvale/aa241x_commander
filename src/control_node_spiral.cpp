...
geometry_msgs::PoseStamped _landing_Pos;
        ...
        float ly = 0.0f;
        float lz = 0.0f;
        float beaconXloc= 50.0f;
        float beaconYloc = 20.0f;
...
//        std::cout<<"x0off: "<<x0off<<", y0off: "<<y0off<<", z0off: "<<z0off<<endl;
        ...
        if (_wp_index == 1) {
            ...
            if(abs(current_x-beaconXloc)<10 && abs(current_y-beaconYloc)<10 && (numBeacon==0) ){
        ...
        if((_wp_index == 2)){
                ...
                denBd = sqrt(pow((beaconXloc- xBeacond),2)+pow((beaconYloc - yBeacond),2));
                uBeacond = 2 *(yBeacond-beaconYloc)/denBd;
                vBeacond = 2 * (beaconXloc- xBeacond)/denBd;
...
//                    std::cout<<"BeaconXloc: "<<beaconXloc<<", BeaconYloc: "<<beaconYloc<<endl;
                ...
                std::cout<<"uBeacon: "<<uBeacond<<", u: "<<_target_Vx<<", vBeacon: "<<vBeacond<<", v: "<<_target_Vy<<endl;
                std::cout<<"thetaBd: "<<thetaBd<<endl;
                std::cout<<"denBd : "<<denBd<<endl;

               if(abs(thetaBd - 2*3.14159)<0.1){
                _wp_index--;
                numBeacon++;
                xd = current_x;
                yd = current_y;
