...
private:
	...
	int _n_waypoints = 1;
        ...
        float _target_U_lake_c = 20.0f; // This is the altitude desired w/r to lagunita frame
        float _target_E_lake_c=0.0f;
        float _target_N_lake_c=0.0f;
	...
	float _line_heading=_pi/6;
        ...
        float lx=0.0f; // POITING SOUTH
        float ly=0.0f; // POITING WEST
        ...
        // Landing position
        float _landing_e_lake=0.0f;
        float _landing_n_lake=0.0f;
...
void  ControlNode::landingPoseCallback(const geometry_msgs::PoseStamped::ConstPtr& msg) {
    ...
    // Update the target
        float _current_local_pos_E_c=_current_local_pos.pose.position.x;
        float _current_local_pos_N_c=_current_local_pos.pose.position.y;
        float _current_local_pos_U_c=_current_local_pos.pose.position.z;
        float _current_local_pos_E_lake_c=_e_offset+_current_local_pos_E_c;
        float _current_local_pos_N_lake_c=_n_offset+_current_local_pos_N_c;
        float _current_local_pos_U_lake_c=_u_offset+_current_local_pos_U_c;

        _target_E_lake_c=_current_local_pos_E_lake_c-ly;
        _target_N_lake_c=_current_local_pos_N_lake_c-lx;
        _target_U_lake_c=_current_local_pos_U_lake_c-lz;
    }
...
//        lz=-_current_local_pos_U+dist(generator);
    ...
    // First waypoint to mimic a take off while already in the air
		...
		// check condition on being "close enough" to the waypoint
			...
			_wp_index++;
                        _target_E_lake=_landing_e_lake;
                        _target_N_lake=_landing_n_lake;
                        _target_U_lake=_u_offset+10;
		}
        }
        ...
        if (_wp_index == 1) {
                ...
                if ((abs(_current_local_pos_U_lake - _target_U_lake) < 0.5) && (abs(_current_local_pos_N_lake - _target_N_lake)< 0.5) && (abs(_current_local_pos_E_lake - _target_E_lake)< 0.5) && (lx!=0 || ly!=0 || lz!=0)) {
                        ...
                        // increment the waypoint to go to the second waypoint "Computing the value of crossing point with line"
                        _wp_index++;
                        _target_E_lake=_target_E_lake_c;
                        _target_N_lake=_target_N_lake_c;
                        _target_U_lake=_target_U_lake_c;
                }
        }
        if (_wp_index == 2) {
                //_target_U_lake=_u_offset+_flight_alt;
...
void ControlNode::missionStateCallback(const aa241x_mission::MissionState::ConstPtr& msg) {
	...
	_u_offset = msg->u_offset;
        _landing_e_lake=msg->landing_e;
        _landing_n_lake=msg->landing_n;
}
...
int ControlNode::run() {
	...
	while (ros::ok()) {
		...
		float _current_local_speed_U=_current_local_speed.twist.linear.z;
                ...
                if(_wp_index == 2 & abs(vel.z)>0.2) { //CONDTION TO MAKE SURE THE DRONE DON'T GO TO FAST WHEN WE RELY ON CAMERA
                    vel.z=-0.2;
                }
