...
//            _target_Vin = 6.5*_target_V/(2*M_PI)*log(den/(den-50));
            _target_Vin = 1.3; //0.001* _target_V/(2*M_PI)*log(3);
            _target_vx = _target_V *(current_y-y0)/den + _target_Vin * (x0-current_x)/den;
            _target_vy = _target_V * (x0 - current_x)/den + _target_Vin * (y0-current_y)/den;
